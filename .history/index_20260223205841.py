...
class BarPOSApp:
    ...
    def new_order(self):
        # If there is an active bill, ask whether to save or discard before starting a new one
        if self.current_bill:
            resp = messagebox.askyesnocancel("New Order", "Save current order before starting a new one?\nYes=Save, No=Discard, Cancel=Keep current")
            if resp is None:
                return  # cancel
            if resp is True:
                # save as unpaid
                self.save_order(paid=False)
            else:
                # discard current
                self.current_bill.clear()
                self.refresh_table()

        # Reset table & waiter and UI for a new order
        self.table_var.set("")
        self.waiter_cb.current(0)
        self.waiter_var.set(self.waiters[0])
        self.hide_pending()
        self.search_var.set("")
        self.search_entry.focus()
